...
**Medidas fluidas abaixo de `lg`.** A coluna visual do hero mede `h-[clamp(14rem,72vw,26rem)]` (560px em `lg`) e o marco da caixa é `size-[30%]` dela (`lg:size-32`), para a caixa estática escalar junto com a bolha; os slots de bolha das estações medem `size-[clamp(13rem,62vw,17rem)]` e sangram de propósito além dos 272px úteis de um viewport de 320 (o `<main>` recorta com `overflow-x-clip`); o mapa mede `size-[clamp(13rem,60vw,17rem)]`. Fora as bolhas que sangram, nenhuma largura fixa em px fica acima de `calc(100vw - 3rem)` em telas pequenas; `html` e `body` levam `overflow-x-clip`.

**Zigue-zague num só eixo (abaixo de `lg`).** O wrapper de texto da `Station` vira `contents`, e título, bolha e corpo passam a ser itens diretos do grid, na ordem título → bolha → parágrafo. A bolha sangra pela borda que alterna com `flip` (`justify-self-end -mr-12` / `justify-self-start -ml-12`, `size-[clamp(13rem,62vw,17rem)]`), preservando o zigue-zague do desktop num único eixo; em `md` (tablet) título e corpo dividem a coluna flexível (`minmax(0,1fr)`) e a bolha ocupa a coluna `auto` nas duas linhas — com `flip` as tracks também espelham, para o texto ficar sempre na coluna flexível. A estação da malha condensa a diagonal D2: mapa no alto-direita e bolha da fábrica em baixo-esquerda dentro de um palco `h-[clamp(20rem,88vw,26rem)]`. O hero segue o mesmo movimento com a bolha `absolute` atrás do título. Ritmo vertical no celular: `py-14` por estação, `gap-y-5` entre título/bolha/corpo, Chegada `pt-14`, rodapé `pt-14` com wordmark `text-6xl` — o vão entre seções é o `py`, nunca uma altura mínima.
