...
**A Regra do Limiar da Jornada.** A coreografia de scroll (caixa viajante + caminho líquido + estouro da bolha) só roda em telas ≥1024px sem `prefers-reduced-motion` (`useJourneyActive`). Abaixo de 1024px o `<Canvas>` **não monta**: `Scene3DLazy` testa `matchMedia("(min-width: 1024px)")` (`useWide`) antes do `import()` de `scene3d`, então o chunk de three.js/R3F não desce ao celular, e as bolhas são imagens pré-renderizadas (ver Bolha-Pacote). Com movimento reduzido em tela larga a página nunca fica num estado parcialmente animado: a caixa fica parada dentro da bolha do hero, o caminhão já nasce estacionado na doca, o SVG do caminho líquido não renderiza e o `<Canvas>` roda em `frameloop="demand"` — um quadro por scroll/resize (`DemandRedraw`) — com o `THREE.Clock` parado (delta 0), então microbolhas, `Float` e o bob da caixa ficam de fato parados. A cena entra por `React.lazy` sem SSR (`Scene3DLazy`) e as constantes que o DOM lê (`JOURNEY_ANCHORS`, `DOCK_HANDOFF_END`) vivem em `journey-constants.ts`, fora de `scene3d.tsx`.
...
- **Do** abaixo de `lg`, colocar o visual acima do título e sangrando pela direita (Regra do Lado Fixo); novo objeto de estação nasce como asset em `public/bolhas/`.
